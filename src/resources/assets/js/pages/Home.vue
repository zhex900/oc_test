<template>
    <v-app id="inspire">
        ...
        <v-toolbar color="teal lighten-1" dark fixed=true>
                    ...
                    <template slot="items" slot-scope="props">
                        <tr @click="props.expanded = !props.expanded">
                            ...
                            <td class="text-xs-left">
                                <div v-if="props.item.membership==null">
                                    <v-btn color="grey" class="white--text" small dark>
                                        Guest
                                        <v-icon>face</v-icon>
                                    </v-btn>
                                </div>
                                <div v-else>
                                    <v-btn :color="membershipColors[props.item.membership.name.toLowerCase()]" class="white--text" small dark>
                            ...
                            <div v-if="props.item.membership!=null">
                                <td class="text-xs-left">{{ `${fdate(props.item.membership.expiry)}, `}}
                                    <span :class="closeToExpire(props.item.membership.expiry)"> {{`${until(props.item.membership.expiry)} left`}}</span>
                                </td>
                            </div>
                        </tr>
                    </template>
...
import { resolve, filter } from 'bluebird';
    ...
    data: () => ({
      ...
      tableSearch: '',
      membershipColors:{
          'silver':'blue lighten-3',
          'gold':'yellow darken-2',
          'platinum': 'indigo accent-4'
      },
      headers: [
          { text: 'ID', value: 'id'},
    ...
    watch: {
        memberOnly(val){
            if(val){ //members only
                this.entries=this.entries.filter(x=>x.membership!=null);
            }else{
        ...
        expireIn30Days(val){
            if(val){ //true only
                this.entries=this.entries.filter(x=>paraseInt(x.datesLeft)<=30);
            }else{
                this.querySelections(val);
            }
        },
