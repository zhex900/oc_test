<template>
    <v-app>
        <v-toolbar :color="filterIndicator[filterOn]" dark fixed>
            ...
            <v-autocomplete v-model="selectedPerson" :items="people" :loading="isLoading" :search-input.sync="search" color="white" cache-items flat hide-no-data hide-details item-text="searchable" item-value="first_name" label="Client Lookup" placeholder="Search by first name" prepend-icon="mdi-database-search" return-object>
        ...
        <v-content style="padding-top: 60px">
            <v-card>
                ...
                <v-data-table :headers="headers" :items="people" :search="tableSearch" :pagination.sync="pagination">
                    ...
                    <template slot="expand" slot-scope="props">
                        <MembershipDetails :membership="props.item"></MembershipDetails>
                    </template>
                </v-data-table>
            </v-card>
        </v-content>
...
export default {
    name: "Home",
    components: {Membership,MembershipDetails,Expiry},
    data: () => ({
        // toolbar
        cachedEntries:[],
        entries: [],
    ...
    computed: {
      ...
      filterOn(){
          return this.filterFirstName.length!=0 || this.filterLastName!=0 || this.filterPhone!=0 || this.filterMemberOnly || this.filterExpireIn30Days;
      },
      people () {
